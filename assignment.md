# [Knack 2 Combat guide](https://stinger.actieforum.com/t235-knack-2 "Where I copied and Pasted")
...
![alt text](https://www.playstation.com/en-us/games/knack-2/ "KNACK")
...
Each form will be detailed in its own section at a later point in this guide, but here I will give a basic overview of each form as the game describes them, as well as a few extra details, as the game does not clearly explain some of them. The colors represent their in-game color, and will be used when listing their moves to differentiate from default Knack's stuff.
...
Explanation: Not entirely true. It can unleash a max of three missiles, but how many come out from a parry is dependent on how many enemies you're facing.
...
You can cancel out of blocking by doing Form Change just after the attack connects. You will also cancel any push-back if timed perfectly. Usefulness is very limited due to not being effective on successive attacks that can hit you while you transform.
...
Like the special forms, they are an addition to default Knack, though these ones DO change certain moves. They can also be used with special forms.
...
Size change dodge
Switching to small Knack at the last second will cause a brief duration of slow-mo. You can also mash the switch button on and off to extend it and get out of continuous attacks. Here is an example of using size change to dodge a single attack.
...
Example: Large robots with missile launchers will fire a horizontal spread of 5 missiles in a straight line. When you're small, they will fire 5 single missiles in succession. So if you ran to the side, they can aim at you while firing single shots making it harder to avoid.

To be honest though, the new attacks are not an issue. It's the speed up that's the real threat. But you can purposely bait enemies into doing these attacks by shrinking. The great thing about this is, you can immediately change back to full size and they will still carry out their attack.
...
During the game you'll encounter robots of various types. These guys have an ability where they cover themselves with electricity. In this state, they can't be attacked physically and their attacks become unblockable. You can cancel this state with certain attacks though.
...
Jump Kick
This is a spinning heel drop. When he lands with this kick, there will be an extended length of relics than come down with his heel and lay out on the ground in front of him. Sort of looks like he's bringing a whip of relics down.
...
It can be done from jump, double jump, hover, Flip Kick, air dodge and Shockwave.



Body Slam
This attack has a small AoE shockwave, but can leave Knack very vulnerable. Once you start it, you can't cancel with anything other than Form Change. Knack plummets directly downward at high speed, creating a splash of relics. You can easily be hurt during it though, especially if enemies start attacking below you as you come down.

It can be done from jump, double jump, hover, air dodge, Flip Kick and Shockwave.

Not a hugely useful move, but can be decent on groups of small enemies when you are larger. It also gets an upgrade that creates a shockwave which can break Sunstone crystals from a decent distance away. This includes the large Sunstone crystal that activates Super Move Mode and will also trigger Sunstone Bomb.
...
I often refer to this move as a dash because that's what it looks like, though the game labels it as 'dodge'. If you ever see me mention dashing, you know I'm talking about dodging.
...
Multi Punch - This cancels the recovery on the final dodge, but also delays you slightly for another reason. MP is a move that requires you to hold the square button and causes Knack to unleash a barrage of punches, and when he does this there's like about half a second of him winding up, so ultimately you end up with a slight delay.

This is still useful because you can also cancel MP with a few things, and do so as soon as you see the move about to come out. Here is an example of using it to cancel the recovery on the final dodge and keep going.
...
This completely deletes the recovery time and allows you to infinitely dodge at ridiculously high speed, tear through levels and run circles around enemies. With the upgrade to make the dodge do damage, you can use this as a high speed attack though it isn't a huge damage dealer. You can still stop and attack at any time.
...
Dodge Delay
This follows the same rules as the punch/kick delay. Knack will enter the fighting stance though it's shorter than p/k, by about 1 second. The next dodge will come out as long as you do it before Knack completely returns to neutral position.
...
Using the Power Punch causes enough damage in a single strike to shatter most armor and shields instantly depending on size, and upon breaking either, will send the enemy sliding back. Lighter attacks still have their use in this scenario, considering you might not want to commit to a Power Punch due to nearby enemies. In this case, other attacks like regular punches and kicks would be the better option due to how highly cancelable they are, offering far more options to avoid damage.
...
-Can be done in the air from single or double jumps, also allowing air options.
...
You can parry, which grants i-frames, then immediately Flip Kick and parry again. This is super useful when dealing with bullet streams like this or multiple shooters.

Hover version
There is a specific point in Hover where you can do a Flip Kick, but it causes the Flip Kick to go downward if at the max height of Hover. I think this is to prevent it from going higher than Knack's max jump height, but it really seems like a glitch due to how it cancels Hover, drops straight down and looks weird, like he's flipping in reverse and then hits the ground too early.

Note: While this version is extremely awkward, and I can't think of any time I would ever use it, it will still deflect projectiles. Even then, you can block and parry in the air from Hover, which is way more useful and easier to do, so there's really no point doing this glitch.
...
Boomerang
This move restrains enemies for a short time, preventing them from moving or attacking. This temporarily removes them from the battle, allowing you to close distance, attack, or focus on other threats. It doesn't do damage, but does have an upgrade that causes damage when Knack hits them while they're restrained. It's generally used for control and is easily one of the most useful moves in the game.

Overview
-Long range.
-Homes in on closest enemy, or wherever Knack is facing.
...
With robots, they lose the ability to block when they activate electricity fields. In this state they are completely vulnerable to Boomerang. It will not only restrain them, but it also cancels their electricity field. One thing I discovered recently though, is that when restrained and still holding a shield, if you throw another Boomerang it will not reset the duration. They will actually just block as if they weren't restrained.
...
Another thing these types can do is push you away if you get too close. While this doesn't do damage, it puts you at a disadvantage by knocking you down. They can then quickly follow up with their attack, forcing you to block and get guard broken. Here is an example.
...
Hitting the crystal will result in Super Mode, where Knack will become invincible and have infinite Sunstone energy for a set amount of time. In this mode, you gain access to super moves.
...
Knack's basic punch and kicks become faster. This allows you to get in more hits before certain enemies try to guard, and interrupt enemy attacks more easily.
...
Air
This can be done any time Knack is in the air from jumping, Hover, or a grounded Shockwave. It cannot be done from Flip Kick, air dodge, or air guard (unless you release guard). It can also be done while falling, like if you ran over a ledge and dropped.
...
Note: The side you appear on depends on the angle you teleported from. Generally you want to appear behind for the best advantage, but sometimes it can be a little off, or maybe your angle was intentional.
...
As for the parry bonus, the extra missiles don't cost Sunstone meter, so even if you run out for regular shots, you will always have this (so long as there is a projectile enemy), which can take out up to three enemies.
...
Crystal Blast
This is Knack's heavy shot. By holding the button, you can rapid fire this move but takes quite a lot of meter. It's basically Multi Punch with projectiles. You can opt to fire only a single shot which could be considered its own move as it may not be necessary to spend the energy on multiple shots.
...
Iron Knack can swing from hooks to cross large gaps, use his extra weight to activate pressure panels to open doors and drop a metal line that can transfer electricity currents. Ice Knack can freeze switches in place to keep gates and elevators open, or to freeze other obstacles.
...
Iron Whip cannot cancel electricity. Strangely, Knack will not take damage when attacking like this. The enemy will not flinch at all or take damage in this state.
...
You can direct the stream to the left or right slightly and catch and enemies off to the side of you. You have to press the direction after the stream starts and you don't get a lot of time to sweep it. You only really have enough time to angle it one way, so check the enemy's positions first.
